  ...
  persist(
    (set, get) => ({
      ...
      initUser: (uid) => {
        ...
        if (existing?.uid === uid) {
          // Persisted profile is already loaded — just mark hydrated
          set({ isHydrated: true });
          return;
        }
        // First launch or UID mismatch — create a fresh profile
      ...
      incrementStreak: () => {
        ...
        const profile = get().profile;
        if (!profile) return;
        if (profile.streak_last_day === today) return;
    ...
    }),
    {
      ...
      // Only persist data — not runtime flags like isHydrated
      partialize: (state) => ({
